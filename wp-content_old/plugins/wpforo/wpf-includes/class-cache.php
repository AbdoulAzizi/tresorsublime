...
class wpForoCache{
	...
	private function init(){
		$uplds_dir = WPFORO_UPLOAD_DIR;
		$cache_dir = WPFORO_CACHE_DIR;
		...
		if( !is_dir($cache_dir . '/item/option') ) $this->mkdir(array($cache_dir . '/item/option'));
	...
	public function get( $key, $type = 'loop', $template = NULL ){
		...
		$loop_templates = array('forum', 'topic', 'post', 'tag', 'option');
	...
	public function clean( $id, $template, $item = array() ){
		...
		elseif( $template === 'post' || $template === 'post-soft' ){
            ...
            if( $parentid ){
                $file = $this->dir . '/item/post/' . $parentid . '_' . $this->lang; $this->clean_file( $file );
            }
            if( $root ){
                $file = $this->dir . '/item/post/' . $root . '_' . $this->lang; $this->clean_file( $file );
            }
			if( $id ){
		...
		elseif( $template === 'option' ){
			$dirs = array( $this->dir . '/item/option/' );
		}
		...
		elseif( $template === 'item' ){
			...
			$dirs = array( $this->dir . '/forum', $this->dir . '/topic', $this->dir . '/post', $this->dir . '/tag', $this->dir . '/item/post', $this->dir . '/item/topic', $this->dir . '/item/forum', $this->dir . '/item/tag', $this->dir . '/item/option/' );
            WPF()->seo->clear_cache();
		}
